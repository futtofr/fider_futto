...
// Roadmap shows all posts grouped by status in a vertical layout
func Roadmap() web.HandlerFunc {
	return func(c *web.Context) error {
		c.SetCanonicalURL("/roadmap")

		allPosts := &query.GetAllPosts{}
		getAllTags := &query.GetAllTags{}
		countPerStatus := &query.CountPostPerStatus{}

		if err := bus.Dispatch(c, allPosts, getAllTags, countPerStatus); err != nil {
			return c.Failure(err)
		}
		...
		return c.Page(http.StatusOK, web.Props{
			...
			Title:       "Roadmap",
			Description: "View our product roadmap and what we're working on",
			Data: web.Map{
